...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    fourthSensor = map(digitalRead(3), 0, 1, 0, 255); 
    fifthSensor = map(digitalRead(4), 0, f1, 0, 255); 
    sixthSensor = map(digitalRead(5), 0, 1, 0, 255);  
    // send sensor values:
    ...
    Serial.print(",");
    Serial.println(fifthSensor, DEC);      
    Serial.print(",");
    Serial.println(sixthSensor, DEC);      
  }
}

void establishContact() {
  while (Serial.available() <= 0) {
    Serial.println("connected");   // send an initial string
    delay(300);
  }
}
